  ...
  <div v-if="isModalSelectUserOpen" class="modal">
    <div class="modal-content" @click.stop>
      ...
      <div class="flex justify-between" style="width: calc(100% - 20px); margin: 15px;">
        <button class="bg-custom-F5A742 hover:bg-orange-700 text-white font-bold py-3 px-6 rounded" style="width: 100px; text-align: center; margin-left:calc(100% - 120px) " @click="publishEvent">발행</button>
      </div>
        </div>
      </div>
    </div>
...
export default {
  props: ['isModalSelectUserOpen', 'selectedEvents'],
  data() {
    return {
      userList: [],
      selectedUsers: [],
...
async sendEmails() {
  console.log(Object.keys(this.selectedEvents));
